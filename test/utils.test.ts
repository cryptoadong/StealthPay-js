...
describe("Utilities", () => {
  describe("Public key recovery", () => {
    it("recovers public keys from type 0 transaction", async () => {
      const hash =
        "0xdd54313d1f1d8211d962207cf939cdc622da9f32a660b18c5df04b1039067c9c";
      const tx = await ethersProvider.getTransaction(hash);
      expect(tx.type).to.equal(0);
      expect(
        await utils.recoverPublicKeyFromTransaction(hash, ethersProvider)
      ).to.equal(
    ...
    it.skip("recovers public keys from type 1 transaction", async () => {
      const hash =
        "0xa75bc0c12658f0fb1cdf501e9395c9cb9e5198c1ea34cbbac6c61caf94076e7c"; // sent with empty access list
      const tx = await ethersProvider.getTransaction(hash);
      ...
      const hash2 =
        "0x9e35a3fbc2951060a169c0ed5a7bc858f2712617f358f8a7386626adca9cea07"; // sent with data in access list
      const tx2 = await ethersProvider.getTransaction(hash2);
      expect(tx2.type).to.equal(1);
      expect(
        await utils.recoverPublicKeyFromTransaction(hash2, ethersProvider)
      ).to.equal(publicKey);
    });

    it("recovers public keys from type 2 transaction", async () => {
      const hash =
        "0xfe80fe73b195eed3874aac3acf8ce7e4b199622bc209bdbdb30b0533bcff5439";
      const tx = await ethersProvider.getTransaction(hash);
      ...
      const hash2 =
        "0x22f07e4fad1329ce6f6cc6cafdc080c23fd9e70822121f634a41ac459b18d1be";
      const tx2 = await ethersProvider.getTransaction(hash2);
  ...
  describe("Recipient identifier lookups", () => {
    ...
    it("throws when looking up recipients by public key without explicitly allowing it", async () => {
      ...
      await expectRejection(
        utils.lookupRecipient(publicKey, ethersProvider),
        errorMsg
      );
    });
    ...
    it("throws when given a public key not on the curve", async () => {
      ...
      await expectRejection(
        utils.lookupRecipient(badPublicKey, ethersProvider, {
          supportPubKey: true,
        }),
        errorMsg
      );
    ...
    it("throws when looking up recipients by transaction hash without explicitly allowing it", async () => {
      ...
      await expectRejection(
        utils.lookupRecipient(hash, ethersProvider),
        errorMsg
      );
    });
    ...
    it("looks up recipients by address, advanced mode on", async () => {
      const ethersProvider = new StaticJsonRpcProvider(
        `https://goerli.infura.io/v3/${String(process.env.INFURA_ID)}`
      );
    ...
    it.skip("looks up recipients by CNS, advanced mode on", async () => {
      ...
      const keys = await utils.lookupRecipient(
        "udtestdev-msolomon.crypto",
        ethersProvider,
        { advanced: true }
      );
      expect(keys.spendingPublicKey).to.equal(pubKeysWallet.spendingPublicKey);
    ...
    it("looks up recipients by address, advanced mode off", async () => {
      const ethersProvider = new StaticJsonRpcProvider(
        `https://goerli.infura.io/v3/${String(process.env.INFURA_ID)}`
      ); // otherwise throws with unsupported network since we're on localhost
      const keys = await utils.lookupRecipient(address, ethersProvider);
      expect(keys.spendingPublicKey).to.equal(
        pubKeysSPayment.spendingPublicKey
      );
      ...
      expect(keys2.spendingPublicKey).to.equal(
        pubKeysSPayment.spendingPublicKey
      );
    ...
    it("looks up recipients by ENS, advanced mode off", async () => {
      ...
      // Same test, but with advanced mode off explicitly specified
      const keys2 = await utils.lookupRecipient(
        "msolomon.eth",
        ethersProvider,
        { advanced: false }
      );
  ...
  describe("Input validation", () => {
    ...
    it("throws when recoverPublicKeyFromTransaction is given a bad transaction hash", async () => {
      ...
      await expectRejection(
        utils.recoverPublicKeyFromTransaction("q", ethersProvider),
        errorMsg
      );
      // @ts-expect-error
      await expectRejection(
        utils.recoverPublicKeyFromTransaction(1, ethersProvider),
        errorMsg
      );
    });
    ...
    it("throws when recoverPublicKeyFromTransaction is given a transaction that does not exist", async () => {
      const mainnetTxHash =
        "0xce4209b4cf80e249502d770dd7f2b19ceb22bbb2cfb49500fe0a32d95b127e81";
      await expectRejection(
    ...
    it("throws when looking up an address that has not sent a transaction", async () => {
      ...
      await expectRejection(
        utils.lookupRecipient(address, ethersProvider),
        errorMsg
      );
    });
