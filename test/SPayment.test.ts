...
describe('SPayment class', () => {
  ...
  beforeEach(async () => {
    ...
    const umbraFactory = new SPayment__factory(deployer);
    const umbraContract = (await umbraFactory.deploy(toll, tollCollector, tollReceiver)) as SPaymentContract;
    ...
    umbra = new SPayment(ethersProvider, chainConfig);
  });
  ...
  describe('Initialization', () => {
    ...
    it('initializes correctly when passing a chain config', async () => {
      // URL provider
      const umbra1 = new SPayment(jsonRpcProvider, chainConfig);
      console.log("umbra1 chainId:", umbra1.chainConfig.chainId);
      console.log("umbra1 umbraAddress:", umbra1.chainConfig.umbraAddress);
      ...
      // Web3 provider
      const umbra2 = new SPayment(ethersProvider, chainConfig);
      ...
      console.log("umbra2 chainId:", umbra1.chainConfig.chainId);
      console.log("umbra2 umbraAddress:", umbra1.chainConfig.umbraAddress);
    ...
    it('initializes correctly when passing a default chainId', async () => {
      // --- Localhost ---
      // URL provider
      // const umbra1 = new SPayment(jsonRpcProvider, 1337);
      // expect(umbra1.chainConfig.umbraAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      // expect(umbra1.chainConfig.startBlock).to.equal(8505089);
      // expect(umbra1.chainConfig.subgraphUrl).to.equal(false);

      // // Web3 provider
      // const umbra2 = new SPayment(ethersProvider, 1337);
      // expect(umbra2.chainConfig.umbraAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      // expect(umbra2.chainConfig.startBlock).to.equal(8505089);
      // expect(umbra2.chainConfig.subgraphUrl).to.equal(false);

      // --- Goerli ---
      // const umbra3 = new SPayment(jsonRpcProvider, 5);
      // expect(umbra3.chainConfig.umbraAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      ...
      // // --- Mainnet ---
      // const umbra4 = new SPayment(jsonRpcProvider, 1);
      // expect(umbra4.chainConfig.umbraAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      ...
      // // --- Optimism ---
      // const umbra5 = new SPayment(jsonRpcProvider, 10);
      // expect(umbra5.chainConfig.umbraAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      ...
      // // --- Polygon ---
      // const umbra6 = new SPayment(jsonRpcProvider, 137);
      // expect(umbra6.chainConfig.umbraAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      ...
      // // --- Arbitrum ---
      // const umbra7 = new SPayment(jsonRpcProvider, 42161);
      // expect(umbra7.chainConfig.umbraAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
  ...
  describe('Send, scan, and withdraw funds', () => {
    ...
    it('Without payload extension: send tokens, scan for them, withdraw them', async () => {
      ...
      const stealthPrivateKey = SPayment.computeStealthPrivateKey(receiver.privateKey, userAnnouncements[0].randomNumber);
  ...
  describe('Input validation', () => {
    ...
    it('throws when initializing with an invalid chainConfig', () => {
      ...
      const umbraAddress = '0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3'; // address does not matter here
      ...
      // @ts-expect-error
      expect(() => new SPayment(ethersProvider, { umbraAddress })).to.throw(errorMsg1);
      // @ts-expect-error
      expect(() => new SPayment(ethersProvider, { umbraAddress: '123', startBlock: '1', subgraphUrl: false })).to.throw(
        errorMsg2
      );
      expect(
        // @ts-expect-error
        () => new SPayment(ethersProvider, { umbraAddress: '123', startBlock: 1, chainId: badChainId, subgraphUrl: false })
      ).to.throw(errorMsg3);
      // @ts-expect-error
      expect(() => new SPayment(ethersProvider, { umbraAddress: '123', startBlock: 1, chainId: 1 })).to.throw(errorMsg4);
      // @ts-expect-error
      expect(() => new SPayment(ethersProvider, { startBlock: 0, chainId: 4, subgraphUrl: false })).to.throw(
    ...
    it('throws when signWithdraw is passed a bad address', async () => {
      ...
      // These error messages come from ethers
      await expectRejection(
        SPayment.signWithdraw(privateKey, 4, umbra.umbraContract.address, badAddress, tokenAddress, goodAddress, '1'),
        'invalid address (argument="address", value="0x123", code=INVALID_ARGUMENT, version=address/5.6.1)'
      );
      await expectRejection(
        SPayment.signWithdraw(privateKey, 4, umbra.umbraContract.address, goodAddress, tokenAddress, badAddress, '1'),
        'invalid address (argument="address", value="0x123", code=INVALID_ARGUMENT, version=address/5.6.1)'
      );
      await expectRejection(
    ...
    it('throws when signWithdraw is passed a bad chainId', async () => {
      ...
      await expectRejection(
        // @ts-expect-error
        SPayment.signWithdraw(privateKey, badChainId, umbra.umbraContract.address, address, tokenAddress, address, '1'),
