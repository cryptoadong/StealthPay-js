...
const jsonRpcProvider = new StaticJsonRpcProvider(
...
describe("SPayment class", () => {
  ...
  const verifyEqualValues = (val1: BigNumberish, val2: BigNumberish) => {
    expect(BigNumber.from(val1).toString()).to.equal(
      BigNumber.from(val2).toString()
    );
  };

  before(async () => {
    // Load signers' mnemonic and derivation path from hardhat config
    const accounts = hardhatConfig.networks?.hardhat
    ...
    sender = ethers.Wallet.fromMnemonic(
      mnemonic as string,
      `${path as string}/${senderIndex}`
    );
    sender.connect(ethers.provider);
    receiver = ethers.Wallet.fromMnemonic(
      mnemonic as string,
      `${path as string}/${receiverIndex}`
    );
  ...
  beforeEach(async () => {
    ...
    const spaymentContract = (await spaymentFactory.deploy(
      toll,
      tollCollector,
      tollReceiver
    )) as SPaymentContract;
    ...
    chainConfig = {
      ...
      subgraphUrl:
        "https://api.thegraph.com/subgraphs/name/scopelift/spaymentpolygon",
    };
  ...
  describe("Initialization", () => {
    ...
    it("initializes correctly when passing a chain config", async () => {
      ...
      console.log(
        "spayment1 spaymentAddress:",
        spayment1.chainConfig.spaymentAddress
      );
      expect(spayment1.provider._isProvider).to.be.true;
      expect(spayment1.chainConfig.spaymentAddress).to.equal(
        chainConfig.spaymentAddress
      );
      expect(spayment1.chainConfig.startBlock).to.equal(chainConfig.startBlock);
      expect(spayment1.chainConfig.subgraphUrl).to.equal(
    ...
    it("initializes correctly when passing a default chainId", async () => {
      ...
      // expect(spayment1.chainConfig.spaymentAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      ...
      // expect(spayment2.chainConfig.spaymentAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      ...
      // expect(spayment3.chainConfig.spaymentAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      ...
      // expect(spayment4.chainConfig.spaymentAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      ...
      // expect(spayment5.chainConfig.spaymentAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      ...
      // expect(spayment6.chainConfig.spaymentAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
      ...
      // expect(spayment7.chainConfig.spaymentAddress).to.equal('0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3');
  ...
  describe("Send, scan, and withdraw funds", () => {
    ...
    it("reverts if sender does not have enough tokens", async () => {
      ...
      await expectRejection(
        spayment.send(sender, dai.address, quantity, receiver.address),
        msg
      );
    });
    ...
    it("Without payload extension: send tokens, scan for them, withdraw them", async () => {
      ...
      const withdrawTxToken = await spayment.withdraw(
        stealthPrivateKey,
        dai.address,
      ...
      verifyEqualValues(
        await dai.balanceOf(destinationWallet.address),
        quantity
      );
      ...
      const withdrawTxEth = await spayment.withdraw(
        stealthPrivateKey,
        ETH_ADDRESS,
        destinationWallet.address
      );
      await withdrawTxEth.wait();
      const withdrawEthReceipt = await ethersProvider.getTransactionReceipt(
        withdrawTxEth.hash
      );
      const withdrawTokenTxCost = withdrawEthReceipt.gasUsed.mul(
        withdrawEthReceipt.effectiveGasPrice
      );
    ...
    it("Without payload extension: send ETH, scan for it, withdraw it", async () => {
      ...
      const destinationWallet = ethers.Wallet.createRandom();
      const withdrawTx = await spayment.withdraw(
        stealthPrivateKey,
        "ETH",
        destinationWallet.address
      );
  ...
  describe("Input validation", () => {
    ...
    it("throws when initializing with an invalid chainConfig", () => {
      const errorMsg1 =
        "Invalid start block provided in chainConfig. Got 'undefined'";
      ...
      const errorMsg4 =
        "Invalid subgraphUrl provided in chainConfig. Got 'undefined'";
      const spaymentAddress = "0xB3c8CF53FE45d8fde16Bca13D9BD60E50Dae36F3"; // address does not matter here

      // @ts-expect-error
      expect(() => new SPayment(ethersProvider)).to.throw(
        "chainConfig not provided"
      );
      // @ts-expect-error
      expect(() => new SPayment(ethersProvider, {})).to.throw(errorMsg1);
      // @ts-expect-error
      expect(() => new SPayment(ethersProvider, { spaymentAddress })).to.throw(
        errorMsg1
      );
      // @ts-expect-error
      expect(
