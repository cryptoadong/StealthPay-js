...
export async function recoverPublicKeyFromTransaction(txHash: string, provider: EthersProvider) {
  ...
  //修改错误:tx.r和tx.s有空时不允许
  if (!tx.r || !tx.s) {
    throw new Error(`Unsupported tx.r and tx.s:${tx.r} ${tx.s}`);
  }
...
export async function lookupRecipient(
  ...
  //如果不是高级模式，获取接收方注册时保存在注册合约里的数据
  if (!advanced) {
    const registry = new StealthKeyRegistry(provider);
    return registry.getStealthKeys(address);
  }

  //如果是高级模式：查找地址交易hash，通过recoverPublicKeyFromTransaction函数找到公钥，此时2个参数是相同的
...
/**
 * @notice Attempt to resolve an ENS name, and return null on failure 根据ENS获取地址必须是主网
 *
...
/**
 * @notice Attempt to resolve a CNS name, and return null on failure　根据CNS获取地址 使用第三方库
 * @param name
 * @returns
 */
