...
import {
  ENS_REGISTRY_ABI,
  FORWARDING_STEALTH_KEY_RESOLVER_ABI,
} from "./constants";
 ...
 */
export const getResolverContract = async (
  name: string,
...
export async function getPublicKeys(name: string, provider: EthersProvider) {
  ...
  if (resolver.address === AddressZero) {
    throw new Error(
      `Name ${name} is not registered or user has not set their resolver`
    );
  }
  ...
  if (keys.spendingPubKey.eq(Zero) || keys.viewingPubKey.eq(Zero)) {
    throw new Error(
      `Public keys not found for ${name}. Please ask them to setup their SPayment account`
    );
  }
