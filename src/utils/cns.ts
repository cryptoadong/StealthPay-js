...
export async function getPublicKeys(
  ...
  const cnsResolver = createContract(
    resolverAddress,
    CNS_RESOLVER_ABI,
    provider
  );
  const [compressedSpendingPublicKey, compressedViewingPublicKey] =
    await cnsResolver.getMany(
      [cnsKeyPathSpending, cnsKeyPathViewing],
      domainNamehash
    );
  if (!compressedSpendingPublicKey || !compressedViewingPublicKey) {
    throw new Error(
      `Public keys not found for ${name}. User must setup their SPayment account`
    );
  }
  // Return uncompressed public keys
  const spendingPublicKey = `0x${Point.fromHex(
    compressedSpendingPublicKey.slice(2)
  ).toHex()}`;
  const viewingPublicKey = `0x${Point.fromHex(
